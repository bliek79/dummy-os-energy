...
EXPECTED_SOURCE_IDS = (
    "do_source_grid_net_power", "do_source_grid_import_power", "do_source_grid_export_power", "do_source_solar_power", "do_source_battery_charge_power", "do_source_battery_discharge_power", "do_source_home_power",
)
EXPECTED_ENERGY_IDS = (
    "do_energy_actual_quarter", "do_energy_history_status", "do_energy_history_days", "do_energy_forecast_model", "do_energy_forecast", "do_energy_forecast_timeline", "do_energy_forecast_next_quarter", "do_energy_forecast_coverage", "do_energy_forecast_confidence", "do_energy_forecast_model_health", "do_energy_forecast_accuracy", "do_energy_forecast_mae", "do_energy_forecast_bias", "do_energy_forecast_evaluation_samples", "do_energy_forecast_quality_by_daypart", "do_energy_forecast_quality_by_day_type", "do_energy_forecast_quality_by_day_type_and_daypart", "do_energy_forecast_quality_by_hour", "do_energy_peak_learning", "do_energy_time_windows", "do_energy_recency_weighting",
)
EXPECTED_DEGREE_DAYS_IDS = (
    "do_degree_days_status", "do_degree_days_history_days", "do_degree_days_temperature_daily", "do_degree_days_daily", "do_degree_days_weighted_daily", "do_degree_days_reference_daily", "do_degree_days_weighted_reference_daily", "do_degree_days_difference", "do_degree_days_weighted_difference", "do_degree_days_last_day",
)
EXPECTED_DEGREE_DAYS_ENTITY_ID_ALIASES = {unique_id: f"sensor.dummy_os_forecast_{unique_id}" for unique_id in EXPECTED_DEGREE_DAYS_IDS}
...
class ReleaseConsistencyTests(unittest.TestCase):
    ...
    def test_source_namespace_is_complete_and_canonical(self) -> None:
        ...
        for unique_id in EXPECTED_SOURCE_IDS: self.assertIn(unique_id, sensor_source)
    ...
    def test_energy_namespace_contains_exact_public_sensor_ids(self) -> None:
        ...
        for unique_id in EXPECTED_ENERGY_IDS: self.assertIn(f'_attr_unique_id = "{unique_id}"', sensor_source)
    ...
    def test_time_windows_bad_alpha1214_id_is_safe_for_in_place_migration(self) -> None:
        migrations_source = (ROOT / "custom_components/dummy_os_data/entity_migrations.py").read_text(); init_source = (ROOT / "custom_components/dummy_os_data/__init__.py").read_text()
    ...
    def test_degree_days_are_registered_sensor_entities(self) -> None:
        degree_source=(ROOT/"custom_components/dummy_os_data/degree_days.py").read_text(); entity_source=(ROOT/"custom_components/dummy_os_data/degree_days_sensor.py").read_text(); sensor_platform=(ROOT/"custom_components/dummy_os_data/sensor.py").read_text(); init_source=(ROOT/"custom_components/dummy_os_data/__init__.py").read_text()
        for unique_id in EXPECTED_DEGREE_DAYS_IDS: self.assertIn(unique_id, entity_source); self.assertIn(f'("sensor", "{unique_id}", "sensor.{unique_id}")', init_source)
        self.assertIn("*build_degree_days_sensors(coordinator)", sensor_platform); self.assertNotIn("hass.states.async_set", degree_source); self.assertIn("_DEGREE_DAYS_RUNTIME_STATE_ALIASES", init_source); self.assertIn('"sensor.do_weighted_degree_days_daily"', init_source); self.assertIn('"sensor.do_heat_degree_days_last_day"', init_source); self.assertGreaterEqual(init_source.count("_async_remove_degree_days_runtime_states(hass)"), 2); self.assertIn("hass.states.async_remove(current_entity_id)", init_source)

    def test_alpha12_degree_days_generated_aliases_are_explicitly_safe(self) -> None:
        self.assertEqual(EXPECTED_DEGREE_DAYS_ENTITY_ID_ALIASES, DEGREE_DAYS_GENERATED_ENTITY_ID_ALIASES); self.assertEqual(10, len(DEGREE_DAYS_GENERATED_ENTITY_ID_ALIASES))
        for unique_id, entity_id in EXPECTED_DEGREE_DAYS_ENTITY_ID_ALIASES.items(): self.assertTrue(MIGRATION_MODULE.is_known_generated_entity_id("sensor", unique_id, entity_id)); self.assertFalse(MIGRATION_MODULE.is_known_generated_entity_id("sensor", unique_id, f"sensor.user_named_{unique_id}"))

    def test_alpha12_degree_days_stale_states_are_explicitly_cleaned(self) -> None:
        init_source=(ROOT/"custom_components/dummy_os_data/__init__.py").read_text()
        for entity_id in EXPECTED_DEGREE_DAYS_ENTITY_ID_ALIASES.values(): self.assertIn(f'"{entity_id}"', init_source)
        self.assertIn("if registry.async_get(entity_id) is not None:", init_source); self.assertIn("hass.states.async_remove(entity_id)", init_source)

    def test_solar_and_weather_display_names_do_not_repeat_dummy_os(self) -> None:
        sensor_source=(ROOT/"custom_components/dummy_os_data/sensor.py").read_text(); solar_source=(ROOT/"custom_components/dummy_os_data/solar_sensor.py").read_text(); self.assertNotIn('"Dummy OS Weather ', sensor_source); self.assertNotIn('f"Dummy OS Weather ', sensor_source); self.assertNotIn('"Dummy OS Solar ', solar_source); self.assertNotIn('f"Dummy OS Solar ', solar_source); self.assertIn('f"DO Weather {label}"', sensor_source); self.assertIn('"DO Solar Source Status"', solar_source)

    def test_identity_migrations_cover_all_source_energy_and_profile_entities(self) -> None:
        init_source=(ROOT/"custom_components/dummy_os_data/__init__.py").read_text(); source_pairs={"do_data_grid_net_power":"do_source_grid_net_power","do_data_grid_import_power":"do_source_grid_import_power","do_data_grid_export_power":"do_source_grid_export_power","do_data_solar_power":"do_source_solar_power","do_data_battery_charge_power":"do_source_battery_charge_power","do_data_battery_discharge_power":"do_source_battery_discharge_power","do_data_home_power":"do_source_home_power"}; energy_pairs={"do_home_actual_quarter":"do_energy_actual_quarter","do_home_history_status":"do_energy_history_status","do_home_history_days":"do_energy_history_days","do_home_forecast_model":"do_energy_forecast_model","do_home_forecast":"do_energy_forecast","do_home_forecast_timeline":"do_energy_forecast_timeline","do_home_forecast_next_quarter":"do_energy_forecast_next_quarter","do_home_forecast_coverage":"do_energy_forecast_coverage","do_home_forecast_confidence":"do_energy_forecast_confidence","do_home_forecast_model_health":"do_energy_forecast_model_health","do_home_forecast_accuracy":"do_energy_forecast_accuracy","do_home_forecast_mae":"do_energy_forecast_mae","do_home_forecast_bias":"do_energy_forecast_bias","do_home_forecast_evaluation_samples":"do_energy_forecast_evaluation_samples","dummy_os_data_energy_peak_learning":"do_energy_peak_learning"}
        for old,new in {**source_pairs,**energy_pairs}.items(): self.assertRegex(init_source,rf'\("sensor", "{old}", "{new}", "sensor\.{new}"\)')
        self.assertIn("new_unique_id=new_unique_id",init_source); self.assertIn("new_entity_id=target_entity_id",init_source)

    def test_bidirectional_grid_contract_is_fixed(self) -> None:
        sensor_source=(ROOT/"custom_components/dummy_os_data/home_input_sensor.py").read_text(); config_flow=(ROOT/"custom_components/dummy_os_data/config_flow.py").read_text(); const_source=(ROOT/"custom_components/dummy_os_data/const.py").read_text(); self.assertIn('CONF_GRID_NET_POWER_ENTITY = "grid_net_power_entity"',const_source); self.assertIn('LEGACY_CONF_GRID_IMPORT_POWER_ENTITY = "grid_import_power_entity"',const_source); self.assertIn('LEGACY_CONF_GRID_EXPORT_POWER_ENTITY = "grid_export_power_entity"',const_source); self.assertIn("CONF_GRID_NET_POWER_ENTITY",config_flow); self.assertIn("CONF_GRID_NET_POWER_ENTITY",sensor_source)
