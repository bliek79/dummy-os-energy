...
def hour(offset, *, action="baseline", grid_charge=0.0, grid_discharge=0.0, soc=50.0):
    ...
    return {"start": start.isoformat(), "end": (start + timedelta(hours=1)).isoformat(), "start_soc_percent": soc, "action": action, "grid_to_battery_kwh": grid_charge, "battery_to_grid_kwh": grid_discharge}


def grid_support(slots=None):
    slots = slots or []
    return {"status": "ready", "valid": True, "grid_charge_triggered": bool(slots), "soc_percent": 50.0, "selected_charge_slots": slots}


def quarter(offset_minutes, energy=0.2, projected=55.0):
    start = NOW + timedelta(minutes=offset_minutes)
    return {"start": start.isoformat(), "end": (start + timedelta(minutes=15)).isoformat(), "allocated_charge_input_kwh": energy, "projected_soc_after_percent": projected}


def test_bridge_merges_contiguous_safety_hours():
    result = build_do_plan_store_bridge(plan72_result=plan72([hour(1, grid_charge=0.4), hour(2, grid_charge=0.3)]), grid_support_result=grid_support(), now=NOW)
...
def test_bridge_merges_contiguous_native_grid_support_slots():
    result = build_do_plan_store_bridge(plan72_result=plan72([]), grid_support_result=grid_support([quarter(60), quarter(75, projected=58.0)]), now=NOW)
    assert result["candidate_count"] == 1
    candidate = result["candidates"][0]
...
def test_grid_support_suppresses_overlapping_trade_charge():
    result = build_do_plan_store_bridge(plan72_result=plan72([hour(1, action="trade_charge", grid_charge=0.5)]), grid_support_result=grid_support([quarter(60), quarter(75)]), now=NOW)
    assert result["candidate_count"] == 1
...
def test_grid_support_supersedes_hourly_safety_charge_even_without_overlap():
    result = build_do_plan_store_bridge(plan72_result=plan72([hour(5, grid_charge=0.5)]), grid_support_result=grid_support([quarter(60), quarter(75)]), now=NOW)
    assert result["candidate_count"] == 1
    assert result["candidates"][0]["reason"] == "grid_support"
    assert any(item["reason"] == "superseded_by_grid_support" for item in result["suppressed_candidates"])


def test_bridge_never_exposes_more_than_three_candidates():
    result = build_do_plan_store_bridge(plan72_result=plan72([hour(1, grid_charge=0.2), hour(3, action="trade_charge", grid_charge=0.2), hour(5, action="trade_discharge", grid_discharge=0.2), hour(7, grid_charge=0.2)]), grid_support_result=grid_support(), now=NOW)
...
def test_past_candidates_are_suppressed_not_silently_used():
    result = build_do_plan_store_bridge(plan72_result=plan72([hour(-1, grid_charge=0.3)]), grid_support_result=grid_support(), now=NOW)
