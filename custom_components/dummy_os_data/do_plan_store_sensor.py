...
class DummyOSShadowPlanStoreRuntime:
    ...
    def __init__(self,coordinator:Any)->None:
        self.coordinator=coordinator; self.hass=coordinator.hass; self.entry_id=coordinator.entry.entry_id
        self._store:Store[dict[str,Any]]=Store(self.hass,STORE_VERSION,f"{DOMAIN}.{self.entry_id}.do_plan_store_shadow")
        self.snapshot:dict[str,Any]=new_store_snapshot(); self.loaded=False; self.persistence_error:str|None=None; self._load_task=None; self._listeners:set[Callable[[],None]]=set(); self.last_bridge_result:dict[str,Any]|None=None
    ...
    def summary(self)->dict[str,Any]:
        ...
        if self.last_bridge_result is not None:
            result["last_bridge_status"]=self.last_bridge_result.get("status"); result["last_bridge_changed"]=self.last_bridge_result.get("changed")
        return result
    def slot(self,slot_id:int)->dict[str,Any]:
        slots=self.snapshot.get("slots",[]) if isinstance(self.snapshot,dict) else []
        if isinstance(slots,list) and 1<=slot_id<=len(slots) and isinstance(slots[slot_id-1],dict):return deepcopy(slots[slot_id-1])
...
class DummyOSPlanStoreBaseSensor(SensorEntity):
    ...
    def __init__(self,runtime:DummyOSShadowPlanStoreRuntime)->None:self.runtime=runtime; self._remove_listener=None
    @property
...
class DummyOSPlanStoreSlotSensor(DummyOSPlanStoreBaseSensor):
    def __init__(self,runtime:DummyOSShadowPlanStoreRuntime,slot_id:int)->None:
        super().__init__(runtime); self.slot_id=slot_id; self._attr_name=f"DO Plan Store Slot {slot_id}"; self._attr_unique_id=f"do_plan_store_slot_{slot_id}"; self._attr_suggested_object_id=f"do_plan_store_slot_{slot_id}"; self._attr_icon=f"mdi:numeric-{slot_id}-circle-outline"
    @property
    def native_value(self)->str:return str(self.runtime.slot(self.slot_id).get("status","blocked"))
    @property
    def extra_state_attributes(self)->dict[str,Any]:
        slot=self.runtime.slot(self.slot_id); slot.update({"shadow_only":True,"operational_plan_store_write":False,"scheduler_invoked":False,"safety_chain_invoked":False,"service_calls_performed":False,"physical_execution_authority":False}); return slot

def build_do_plan_store_sensors(coordinator:Any,runtime:DummyOSShadowPlanStoreRuntime|None=None)->list[SensorEntity]:
    runtime=runtime or DummyOSShadowPlanStoreRuntime(coordinator)
    return [DummyOSPlanStoreSensor(runtime),*[DummyOSPlanStoreSlotSensor(runtime,slot) for slot in range(1,SLOT_COUNT+1)]]
